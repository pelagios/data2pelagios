...
with open('api-key.txt', 'r') as f:
  ...
  for photo in flickr.walk(user_id = user_id, extras = 'machine_tags,date_taken,geo'):
    ...
    if len(pleiades_tags) > 0:
      ctr_with_pleiades += 1
      output.write('<http://example.org/flickr/dump.ttl#photos/' + photo_id + '> a pelagios:AnnotatedThing ;\n')
      output.write('  dcterms:title "' + photo.get('title').replace('"', '\\"').encode('utf-8') + '" ;\n');  
      output.write('  foaf:homepage <http://www.flickr.com/photos/' + user_id + '/' + photo_id + '> ;\n')
      
      # Note: image at 'https://farm' + photo.get('farm') + '.staticflickr.com/' + photo.get('server') + '/' + photo.get('id') + '_' + photo.get('secret') + '.jpg'   

      if photo.get('datetaken'):
        year = photo.get('datetaken').split('-', 1)[0]
        output.write('  dcterms:temporal "start=' + year + '" ;\n')
      ...
      for idx, pleiades_tag in enumerate(pleiades_tags):  
        output.write('<http://example.org/flickr/dump.ttl#photos/' + photo_id + '/annotations/' + str(idx) + '> a oa:Annotation ;\n')
        output.write('  oa:hasTarget <http://example.org/flickr/dump.ttl#photos/' + photo_id + '> ;\n')
